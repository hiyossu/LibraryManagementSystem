...
/**
 * Builds and returns the Board Game Registration form card.
 * Only Title and Game Type fields — ctx.txtGenre and ctx.txtDewey are set to null.
 */
public class BoardGameFormCard {

    private static final Color ACCENT = new Color(58, 130, 80); // green stripe
    ...
    public BoardGameFormCard(FormContext ctx, UIFactory ui,
                             Color gold, Color cardColor,
    ...
    public JPanel build(Runnable onAdd, Runnable onClear, Runnable onDash) {
        JPanel card = new JPanel(new GridBagLayout()) {
            protected void paintComponent(Graphics g) {
                ...
                g.setColor(ACCENT);       // Green stripe distinguishes Board Game
        ...
        ctx.txtGenre = null;    // not captured for BoardGame
        ctx.txtDewey = null;    // not applicable

        ui.addRow(form, fc, "Game Title", ctx.txtTitle, 0);
        ui.addRow(form, fc, "Game Type",  ctx.txtType,  1);

        fc.gridy = 2; fc.gridx = 0; fc.gridwidth = 4;
